...
const GroupListTable = () => {
  // Get state and actions from the group store
  const {
    groups = [], // Default to empty array if groups is undefined
    loading,
    error,
    successMessage,
    fetchUserGroups,
  ...
  const formattedGroups = safeGroups.map(group => ({
    ...
    members: `${group.members?.length || 0}/${group.maxMembers || 15}`,
    timeLeft: calculateTimeLeft(group.nextPayoutDate),
    progress: calculateProgress(group),
    amount: `$${(group.savingsAmount || 0) * (group.members?.length || 0)}`
  }));
  ...
  // (including the JSX with the table and other UI elements)
